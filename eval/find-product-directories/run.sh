#!/usr/bin/env bash
# Trigger eval for the find-product-directories skill.
#
# Usage:
...
# Skill tool with skill="find-product-directories". A query passes when the trigger
# rate matches its should_trigger label (>50% of runs for positives,
# <=50% for negatives).
...
SPLIT="${1:-train}"
RUNS="${2:-3}"
SKILL="find-product-directories"
